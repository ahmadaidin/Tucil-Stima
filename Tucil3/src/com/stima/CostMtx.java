 ...
 */
public class CostMtx {
    private int[][] mtx;
    private int cost;
    private int size;
    private static final int INFINITY = 999;

    public CostMtx(int size){
        this.size = size;
        cost = 0;
        mtx = new int [size][size];
    }

    public CostMtx(CostMtx costMtx){
        this.size = costMtx.size;
        cost = costMtx.cost;
        this.mtx = new int [size][size];
        for(int i=0; i<size; i++){
            for(int j=0; j<size; j++){
                this.mtx[i][j] = costMtx.getMtx()[i][j];
            }
        }
    }

    public CostMtx(int[][] mtx, int size, int cost){
        this.size = size;
        this.cost = cost;
        this.mtx = new int [size][size];
        for(int i=0; i<size; i++){
